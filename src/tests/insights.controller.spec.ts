...
describe('InsightsController', () => {
  ...
  it('SalesToday should return today sales message', () => {
    ...
    expect(res.json).toHaveBeenCalledWith({ message: "See today's menu sales." });
  });

  it('SalesThisWeek should return week sales message', () => {
    const res = { json: jest.fn() } as any;
    controller.SalesThisWeek(res);
    expect(res.json).toHaveBeenCalledWith({ message: "See counts of sales for every day in this week." });
  });

  it('SalesThisMonth should return month sales message', () => {
    const res = { json: jest.fn() } as any;
    controller.SalesThisMonth(res);
    expect(res.json).toHaveBeenCalledWith({ message: "See counts of sales for every day in this month." });
  });
});
